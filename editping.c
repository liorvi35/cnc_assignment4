...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        printf("%d bytes has been sent from %s to %s: icmp_seq=%d ttl=%d time=%.2f ms\n", 
            len, inet_ntoa(*(struct in_addr*)&ip->saddr),  inet_ntoa(*(struct in_addr*)&ip->daddr), icmp.un.echo.sequence , ip->ttl, time);
